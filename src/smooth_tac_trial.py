...
class Finger(object):
    ...
    def CalcStatus(self):
        if (self.offset+self.Input-5 < self.pr and self.pr < self.offset+self.Input+5):
            self.status = 0
        elif (self.pr >= self.offset+500 or self.Input == 0):
            self.status = -1
        else:
            self.status = 1
    ...
    def CalcNextPos(self,incr):
        if (self.status == -1):
            ...
            self.pos_n = incr*(self.offset+self.Input-self.pr)/200
